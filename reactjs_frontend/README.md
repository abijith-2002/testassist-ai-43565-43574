...
- The correct API base URL is:
  ```
  https://vscode-internal-12911-beta.beta01.cloud.kavia.ai:3001
  ```
- All backend API calls will use `${REACT_APP_API_BASE_URL}` for their base; UI POSTs chat requests to `${REACT_APP_API_BASE_URL}/chat`.
- Set the URL by creating or editing a `.env` file in your `reactjs_frontend/` directory:
  ```
  REACT_APP_API_BASE_URL=https://vscode-internal-12911-beta.beta01.cloud.kavia.ai:3001
  ```
  Or launch with:
  ```bash
  REACT_APP_API_BASE_URL=https://vscode-internal-12911-beta.beta01.cloud.kavia.ai:3001 npm start
  ```
